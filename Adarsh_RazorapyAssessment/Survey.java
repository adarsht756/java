...
public class Survey {
    ...
    public Survey(String surveyName, int id, List<Question> questions) {
    ...
    public void addUserResponse(String username, SurveyResponse surveyResponse) {
        if (responseStore.containsKey(username) == true) {
            System.out.println("User cannot answer survey again");
            return;
        }
        updateSurveyRating(surveyResponse);
        responseStore.put(username, surveyResponse);
